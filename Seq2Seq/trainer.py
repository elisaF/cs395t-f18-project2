"""Implementing training routine for Transformer (Vaswani/17).
...
def get_batch(vocab_size, batch_size, number_batches, data_iterator):
    ...
    for i in range(number_batches):
        source, target = data_iterator.random_batch(batch_size)
        source, target = source.to(DEVICE), target.to(DEVICE)
        yield Batch(source, target, pad=0)
        

def make_model(source_vocab_size, target_vocab_size, 
    ...
    model = EncoderDecoder(
        ...
        nn.Sequential(Embeddings(embed_size, target_vocab_size, glove_init), 
                      deep_copy(position)),
        Generator(embed_size, target_vocab_size)).to(DEVICE)
...
def run_epoch(batches, model, indexer, data_iterator, 
    ...
    for i, batch in enumerate(batches):
        ...
        outputs = model.forward(batch.source, batch.target, 
...
def random_predict(model, indexer, data_iterator, size=5):
    ...
    for i in range(size):
        source, target = data_iterator.random_batch(1)
        # Make mask: <batch-size=1, 1, seq-length>
        source_mask = Variable(torch.ones(1, 1, source.size(1))).to(DEVICE)
        source, target = source.to(DEVICE), target.to(DEVICE)
        prediction = greedy_decode(model, source, source_mask, 
                                   max_length=target.size(1), start_symbol=2)
...
def train(data_dir, model_dir, indexer_dir, glove_path,
    ...
    # Load data
    print("Loading data\n")
    indexer, train_source, train_target, \
    valid_source, valid_target = load_corpus(data_dir)
    print("Data loaded!\n")
    ...
    train_iterator = DataIterator(train_source,train_target,
                                  len(train_source))
    valid_iterator = DataIterator(valid_source, valid_target,
                                  len(valid_source))
    model = make_model(source_vocab_size=vocab_size, target_vocab_size=vocab_size, 
    ...
    for epoch in range(number_epochs):
        ...
        train_loss = run_epoch(get_batch(vocab_size, batch_size, number_batches,
        ...
        if validate:
            ...
            valid_losses = []
            valid_loss = run_epoch(get_batch(vocab_size, 
                                             batch_size, 
                                             number_batches,
...
class TransformerRunner(object):
    ...
    def __init__(self, model_path, indexer_path, glove_path,
        ...
        self.model = make_model(vocab_size, vocab_size, 
    ...
    def paraphrase(self, sentence, max_length=20, pretty_print=True):
        ...
        length = len(tokens)
        source = Variable(torch.LongTensor(np.array([tokens])))
        # Make mask: shape = <batch-size, 1, seq-length>
        source_mask = Variable(torch.ones(1, 1, source.size(1)))
        source, source_mask = source.to(DEVICE), source_mask.to(DEVICE)
        prediction = greedy_decode(self.model, source, source_mask, 
